  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        onClick={handleOpen}
        className="relative w-10 h-10 flex items-center justify-center rounded-xl bg-gray-50 hover:bg-blue-50 border border-gray-200 hover:border-blue-200 transition-all cursor-pointer text-gray-500 hover:text-blue-600"
        title="Notifications"
      >
        <BellIcon />
        {unreadCount > 0 && (
              ...
              notifications.map(n => (
                ...
                >
                  <span className="mt-0.5 flex-shrink-0">
                    {n.type === 'approved' ? <CheckIcon /> : <span className="text-rose-400"><XIcon size={4} /></span>}
                  </span>
...
export default function DashboardLayout({ role }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50" style={{ display: 'block' }}>
      ...
      {/* Sidebar — always fixed on mobile, static column on md+ */}
      <aside className={`
        fixed inset-y-0 left-0 w-64 bg-white border-r border-gray-200 flex flex-col shadow-2xl z-[80] transition-transform duration-300 ease-in-out
        md:fixed md:translate-x-0 md:shadow-sm
      ...
      `}>
        <div className="p-6 flex items-center justify-between border-b border-gray-100">
          ...
          <button
            className="p-2 text-gray-400 hover:text-gray-600 md:hidden cursor-pointer"
            onClick={() => setIsSidebarOpen(false)}
          >
            <XIcon size={5} />
          </button>
      ...
      {/* Main Content — always full width on mobile, offset by sidebar on md+ */}
      <div className="md:pl-64 flex flex-col min-h-screen">
        ...
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-8 shadow-sm sticky top-0 z-50">
          <div className="flex items-center gap-4">
            <button
              className="p-2 text-gray-500 hover:bg-gray-50 rounded-lg md:hidden transition-colors cursor-pointer"
